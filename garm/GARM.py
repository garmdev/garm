...
def calcCDMatrix(model_type,gen_list, maps_list, con_model_user_file,grid_filename,log_handle,msg_verbose,log):
	...
	for i, member in enumerate(gen_list):
		if member.cd_matrix == None:
			...
			if model_type.lower() == 'least_cost': 
				...
				print 'Unknown connectivity model specified, check GARM config file.'
				sys.exit(-1)				
...
def main(*arguements):
	...
	while gen_num < max_gens and top_fitness < 1.0:
		...
		mutation(children_parent_list,m_rate,weight_dict,previous_pop_dict,all_members_list,\
			maps_list,con_model_user_file,grid_filename,log_handle,msg_verbose,\
			genetic_dist_ary, gen_counts_ary,ed_dist_ary, test_type,R_lib_home,test_nperms,model_type,log)
